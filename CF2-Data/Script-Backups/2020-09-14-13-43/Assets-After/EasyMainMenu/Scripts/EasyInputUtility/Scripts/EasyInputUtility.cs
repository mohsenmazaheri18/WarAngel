    ...
    public class EasyInputUtility : MonoBehaviour
    {
        ...
        private void FixedUpdate()
        {
            for (int i = 0; i < Axes.Count; i++)
            {
                ...
                a.negative = ControlFreak2.CF2Input.GetKey(a.negativeKey);
                a.positive = ControlFreak2.CF2Input.GetKey(a.positiveKey);
        ...
        public bool GetButton(string name)
        {
            ...
            for (int i = 0; i < Axes.Count; i++)
            {
                if (string.Equals(Axes[i].axisName, name))
                {
                    ...
                    retVal = ControlFreak2.CF2Input.GetKey(Axes[i].positiveKey);
        ...
        public bool GetButtonDown(string name)
        {
            ...
            for (int i = 0; i < Axes.Count; i++)
            {
                if (string.Equals(Axes[i].axisName, name))
                {
                    retVal = ControlFreak2.CF2Input.GetKeyDown(Axes[i].positiveKey);
                }
            }
