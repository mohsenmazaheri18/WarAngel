...
public class ProjectileScript : MonoBehaviour
{
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        //Explosion force
        Vector3 explosionPos1 = transform.position;
        //Use overlapshere to check for colliders in range
        Collider[] colliderss = Physics.OverlapSphere(explosionPos1, radius);
        foreach (Collider hit in colliderss)
        {
            ...
            //Add force to nearby rigidbodies
            if (rb != null)
                rb.AddExplosionForce(power * 50, explosionPos1, radius, 3.0F);
