...
public class HandgunScriptLPFP : MonoBehaviour {
    ...
    //gun set active true or false
    public GameObject Gun1;
    public GameObject Gun2;

    public GameObject change_gun1;
    public GameObject change_gun2;

    //grenade count
    private int count_grenade = 5;
    public GameObject grenade;
    public GameObject blockgrenade;
	...
	private void Update () {
        ...
        {
            change_gun1.SetActive(false);
            change_gun2.SetActive(true);
            Gun1.SetActive(true);
            Gun2.SetActive(false);
            timescaleText.text = "کلت";
        }
        //Change timesccale to 50% when 2 key is pressed
        if (ControlFreak2.CF2Input.GetKeyDown(KeyCode.Alpha2))
        {
            change_gun1.SetActive(true);
            change_gun2.SetActive(false);
            Gun1.SetActive(false);
            Gun2.SetActive(true);
            timescaleText.text = "کلاش";
        }
		...
		if (ControlFreak2.CF2Input.GetKeyDown (KeyCode.G) && !isInspecting) 
		{
            ...
            count_grenade -= 1;
            grenade_text.text = count_grenade.ToString();

            if (count_grenade <= 0)
            {
                grenade.SetActive(false);
                blockgrenade.SetActive(true);
            }
		}
		...
		//Shooting 
		if (ControlFreak2.CF2Input.GetMouseButtonDown (0) && !outOfAmmo && !isReloading && !isInspecting && !isRunning) 
		{
			anim.Play ("Fire", 0, 0f);
			if (!silencer) 
			{
