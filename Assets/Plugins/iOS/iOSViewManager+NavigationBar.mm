...
extern "C"
{
	...
	void _setItems_navbar(int tag, int * items, int arrayCount, bool animated)
	{
        UINavigationBar *navbar = [MHTools getActualObject:tag];
        if(navbar)
        {
            [navbar setItems:[MHTools convertMultipleTagsToNSArray:items num:arrayCount] animated:animated];
            return;
        }
	...
	void _items_navbar_set(int tag, int * items, int arrayCount)
	{
        UINavigationBar *navbar = [MHTools getActualObject:tag];
        if(navbar)
        {
            navbar.items = [MHTools convertMultipleTagsToNSArray:items num:arrayCount];
            return;
        }
